...
public:
  ...
  EdgeXYZ_Kinematics(double targetValue, double kSpring=2.0, std::string tag="None"):
   targetValue(targetValue), kSpring(kSpring), tag(tag)
  {
    this->setMeasurement(0.);
    this->resize(3);
  }
  ...
  static double lost_calc(
    VertexXYZ* conf1, VertexXYZ* conf2, VertexXYZ* conf3, double targetValue, double kSpring=2.0, bool debug=true
  ){
    Eigen::Vector3d deltaS1 = conf2->position() - conf1->position();
    Eigen::Vector3d deltaS2 = conf3->position() - conf2->position();
    ...
    if (debug){
      std::cout << "  Kinematic EdgeLoss:" << loss << " targetValue:" << targetValue << " ComputeValue:" << cosTheta;
      std::cout << " targetAngle:" << std::acos(targetValue) / M_PI * 180.0 << " ComputeTheta:" << theta / M_PI * 180.0;
...
public:
  ...
  EdgeXYZ_VertexKinematics(Eigen::Vector3d orientation, double targetValue, double kSpring=2.0, std::string tag="None"): 
    orientation(orientation), targetValue(targetValue), kSpring(kSpring), tag(tag)
  {
    this->setMeasurement(0.);
  }
  ...
  static double lost_calc(
    VertexXYZ* conf1, VertexXYZ* conf2, Eigen::Vector3d orientation, double targetValue, double kSpring=10.0, bool debug=true
  ){
    Eigen::Vector3d deltaS1 = conf2->position() - conf1->position();
    ...
    if (debug){
      std::cout << "  Kinematic VertexLoss:" << loss << " targetValue:" << targetValue << " ComputeValue:" << cosTheta;
      std::cout << " targetAngle:" << std::acos(targetValue) / M_PI * 180.0 << " Theta:" << theta / M_PI * 180.0;
