...
void KDTreeWrapper::nearest_range(double x, double y, double z, double bound, std::vector<KDTreeRes*>& resList){
    double pos[3] = {x, y, z};
    ...
    while(!kd_res_end(treeRes)){
        data = (KDTreeData*)kd_res_item(treeRes, pos);

        KDTreeRes* res = new KDTreeRes();
        res->x = pos[0];
        res->y = pos[1];
        res->z = pos[2];
        res->data = data;
