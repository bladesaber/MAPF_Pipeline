...
state_problem = create_state_problem(
    name='coarse_1', F_form=F_form, state=up, adjoint=vq, is_linear=False,
    bcs_info=bcs_info,
...
opt_problem = OptimalShapeProblem(
    ...
    shape_regulariztions=ShapeRegularization([
        VolumeRegularization(control_problem, mu=0.1, target_volume_rho=1.0)
    ]),
...
while True:
    ...
    success_flag, stepSize = deformation_handler.move_mesh_by_line_search(
        displacement_np, max_iter=10, init_stepSize=0.1, stepSize_lower=1e-3,
...
# grid = VisUtils.convert_to_grid(domain)
# pyvista.save_meshio(os.path.join(proj_dir, 'last_model.vtk'), grid)
