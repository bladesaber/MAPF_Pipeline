...
class LowReFluidFineModel(FineModel):
    ...
    def save_anchor(
        ...
        # MeshUtils.save_XDMF(
        #     os.path.join(record_dir, f"{name}.xdmf"), self.domain, self.cell_tags, self.facet_tags
        # )
...
class FluidCoarseModel(CoarseModel):
    ...
    def set_best_parameter(self, domain: dolfinx.mesh.Mesh):
        self.parameter_optimal = self._extract_parameter(domain)
    ...
    def save_anchor(self, name: str = 'opt_model'):
        ...
        # MeshUtils.save_XDMF(
        #     os.path.join(self.record_dir, f"{name}.xdmf"), self.control, self.cell_tags, self.facet_tags
        # )
...
class FluidParameterExtraction(ParameterExtraction):
    ...
    def save_anchor(self, record_dir, name: str = 'para_model'):
        ...
        # MeshUtils.save_XDMF(
        #     os.path.join(record_dir, f"{name}.xdmf"), self.control, self.cell_tags, self.facet_tags
        # )
...
# opt_coarse_domain, _, _ = MeshUtils.read_XDMF(
#     file='/home/admin123456/Desktop/work/topopt_exps/fluid_shape4/coarse_model/opt_model.xdmf',
#     mesh_name='model', cellTag_name='model_cells', facetTag_name='model_facets'
# )
# coarse_model.set_best_parameter(opt_coarse_domain)
...
MeshUtils.save_XDMF(
    os.path.join(proj_dir, "last_model.xdmf"), fine_model.domain, fine_model.cell_tags, fine_model.facet_tags
)
