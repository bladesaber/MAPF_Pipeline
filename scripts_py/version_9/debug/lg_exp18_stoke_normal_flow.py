...
# target_goal_dict = {f"marker_{5}": 0.47, f"marker_{6}": 0.74, f"marker_{7}": 0.79}
...
outflow_datas = {}
for marker in output_markers:
    outflow_datas[f"marker_{marker}"] = {'form': dolfinx.fem.form(ufl.dot(u, n_vec) * ds(marker))}
...
while True:
    ...
    if success_flag:
        ...
        outflow_cells = {}
        for key in outflow_datas.keys():
            data = outflow_datas[key]
            outflow_cells[key] = AssembleUtils.assemble_scalar(data['form'])
