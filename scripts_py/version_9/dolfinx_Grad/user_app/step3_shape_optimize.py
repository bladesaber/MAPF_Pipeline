...
def load_simulator(
        ...
        simulate_method: str, load_guess_res: bool,
...
def load_base_model(
        cfg: dict, simulate_method: str, load_guess_res: bool,
        with_debug: bool, tag_name=None
):
    ...
    simulator = load_simulator(
        cfg, domain, cell_tags, facet_tags,
        simulate_method=simulate_method, load_guess_res=load_guess_res
    )
...
def run_shape_optimize(
    ...
    for run_cfg_file in json_files:
        with open(run_cfg_file, 'r') as f:
            run_cfg: dict = json.load(f)

        if not run_cfg.get('recombine_cfgs', False):
            model, log_dict = load_base_model(
                run_cfg, simulate_method=simulate_method, load_guess_res=load_guess_res,
                with_debug=with_debug, tag_name=None
            )
            log_list.append(log_dict)

        else:
            if len(run_cfg['recombine_cfgs']) == 0:
                continue

            if len(run_cfg['recombine_cfgs']) == 1:
                ...
                for sub_cfg_name in run_cfg['recombine_cfgs']:
                    ...
                    sub_model, log_dict = load_base_model(
                        run_cfg, simulate_method=simulate_method, load_guess_res=load_guess_res,
                        with_debug=with_debug, tag_name=model.tag_name
                    )
