...
from ..lagrange_method.type_database import create_shape_problem, create_state_problem
...
class FluidShapeOptSimple(object):
    def __init__(
        ...
        if isStokeEqu:
            self.F_form = (
                    inner(grad(self.u), grad(v)) * ufl.dx
                    - self.p * div(v) * ufl.dx
                    - q * div(self.u) * ufl.dx
                    - inner(f, v) * ufl.dx
            )
    ...
    def init_state_problem(
        ...
        state_problem = create_state_problem(
            name='coarse_1', F_form=self.F_form, state=self.up, adjoint=self.vq, is_linear=self.is_linear_state,
            bcs_info=bcs_info_state,
            state_ksp_option=state_ksp_option, adjoint_ksp_option=adjoint_ksp_option
        )
        self.state_system = StateProblem([state_problem])

        self.control_problem = create_shape_problem(
            domain=self.domain, bcs_info=bcs_info_control, gradient_ksp_option=gradient_ksp_option
        )
    ...
    def set_logger_dicts(self, logger_dicts: Dict):
        self.logger_dicts = logger_dicts

    def log_step(self, log_recorder, u_recorder, step):
        for tag_name in self.logger_dicts:
            inspectors_dict = self.logger_dicts[tag_name]
    ...
    def solve(self, record_dir, max_iter=200, **kwargs):
        ...
        weight_handler.compute_weight(self.cost_weights)
        for cost_func in self.opt_problem.cost_functional_list:
        ...
        self.log_step(log_recorder, u_recorder, step=0)
        ...
        while True:
            ...
            if success_flag:
                ...
                self.log_step(log_recorder, u_recorder, step)
