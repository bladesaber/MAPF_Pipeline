...
class MiniumSpanTree_TaskRunner(object):
    def __init__(self):
        self.graph = nx.Graph()
...
class MiniumDistributeTree_TaskRunner(object):
    def __init__(self):
        self.names = []
        self.poses = []
    ...
    def getTaskTrees(self):
        ...
        for i in range(size):
            for j in range(i + 1, size, 1):
                dist = self.compute_dist(poses[i], poses[j])
                dist_mat[i, j] = dist
                dist_mat[j, i] = dist
...
class SizeTreeTaskRunner(object):
    ...
    def compute_leaf_dist(self, leaf0, leaf1):
        poses_0, poses_1 = np.array(leaf0['poses']), np.array(leaf1['poses'])
        names_0, names_1 = leaf0['names'], leaf1['names']
        ...
        for _ in range(size - 1):
            ...
            for tag0 in tags_0:
                for tag1 in tags_1:
                    ...
                    selectIdx_i, selectIdx_j, dist = self.compute_leaf_dist(leaf0, leaf1)
                    if dist < best_dist:
                        best_dist = dist
                        pairLeaf_tag0 = tag0
                        pairLeaf_tag1 = tag1
