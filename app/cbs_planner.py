...
class CBSSolver(object):
    def __init__(self, config, agentInfos: Dict):
        self.instance = mapf_pipeline.Instance(config['x'], config['y'], config['z'])
        self.instance.info()
        ...
        for agentIdx in self.agent_idxs:
            ...
            group_idxs.append(groupIdx)
        self.group_idxs = list(set(group_idxs))
        self.num_of_groups = len(self.group_idxs)
    ...
    def solve(self, staticObs_df:pd.DataFrame):
        ...
        for a1 in self.agent_idxs:
            ...
            for a2 in self.agent_idxs:
                if self.agentInfos[a1]['groupIdx'] != self.agentInfos[a2]['groupIdx']:
                    constrains.append((
        ...
        if success_node is not None: 
            ...
            for agentIdx in success_node.agentMap.keys():
                ...
                res[agentIdx]= agent.getDetailPath()
            return res
        
        else:
            print('[DEBUG]: Search Out of Resources')
            return {'status': False}
    ...
    def extendNode(self, node):
        ...
        for agentIdx, constrain in new_constrains:
            success, new_node = self.createCBSNode(node, agentIdx, constrain)
    ...
    def print_NodeGraph(self, node, select_agentIdx=None, obs_mesh=None):
        ...
        for agentIdx in node.agentMap.keys():
            ...
            if agent.findPath_Success:
                ...
                vis.plot(tube_mesh, color=tuple(random_colors[agent.groupIdx]))
            ...
            vis.plot(
                ...
                color=tuple(random_colors[agent.groupIdx])
            )
            vis.plot(
                vis.create_box(
